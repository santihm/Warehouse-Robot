...
import rospy
from warehouse_robot.msg import Task, RobotStatus2, WarehouseMap2
...
paused = True  # Estado inicial del robot pausado
...
def navigate_to(destination, point_type):
    ...
    for step in path:
        ...
        robot_speed = rospy.get_param('~robot_speed', 1.0)  # Leer el parámetro dinámicamente
        time.sleep(1 / robot_speed)  # Esperar según la velocidad del robot
    ...
    if point_type == "source" and current_task is not None:
        ...
        current_task = None  # Marcar la tarea como completada
...
def pause_resume_callback(req):
    global paused
    paused = req.pause
    message = "Robot pausado" if paused else "Robot reanudado"
...
def robot_node():
    ...
    # Iniciar el nodo
    rospy.init_node('robot_node5', anonymous=True)

    # Leer parámetros
    robot_speed = rospy.get_param('~robot_speed', 1.0)
    ...
    # Servicio para pausar/reanudar el robot
